...
bool parseCommand(const std::string& input, std::ostream& output,
	...
	if (input.length()) {
		...
		// Print available commands
			...
			os << "add_profile [profile name] \t\t add a new profile" << std::endl;
			os << "remove_profile [profile name] \t\t remove an existing profile" << std::endl;
			os << "set_profile [profile name] [userID] \t set an existing profile to a certain user ID" << std::endl;
			os << "get_profile [profile name] [userID] \t get the binary profile of a certain user ID" << std::endl;
			...
			if (commands.size() == 2) {
				...
				if (!profileManager.isExist(profileName)) {
					...
					wrongArgument = !profileManager.insertProfile(
													profileName, baseProfile, baseProfileSize);
		...
		// Extract and use an existing profile for a particular headset
			...
			if (commands.size() == 3) {
				...
				if (convert(commands.at(2), userID) && profileManager.isExist(profileName)) {
					...
					if (profileManager.extractProfile(profileName, profileBuffer, &profileSize)) {

						// Set the binary profile into EmoEngine
						wrongArgument = EE_SetUserProfile(userID, profileBuffer, profileSize) != EDK_OK;

						if (!wrongArgument) {
							headsetProfileMap[userID] = profileName;
							os << "Profile [" << profileName << "] set with the headset " << userID << ".";
						}
					}
			...
			if (commands.size() == 3) {
				...
				if (convert(commands.at(2), userID) && profileManager.isExist(profileName)) {
					...
					if (EE_GetUserProfile(userID, eProfile) == EDK_OK) {
						...
						if (profileHandleToByteArray(eProfile, &profileBuffer, &profileSize)) {
							...
							if (iter != headsetProfileMap.end()) {
								...
								if (!profileName.empty()) {
									...
									os << "Profile [" << profileName << "] acquired successfully.";
									wrongArgument = false;
								}
							}
						}
		...
		// List existing mapping of headsets and profiles
			...
			if (commands.size() == 1) {
				...
				std::map<unsigned int, std::string>::const_iterator iter = headsetProfileMap.begin();

				for ( ; iter != headsetProfileMap.end(); ) {
					const std::string& profileName = iter->second;
					os << "headset " << iter->first << " : ";
					os << (profileName.empty() ? "[no profile associated]" : profileName);
					iter++;
					if (iter != headsetProfileMap.end()) {
						os << std::endl;
					}
