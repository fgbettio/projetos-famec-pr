...
int main(int argc, char **argv) {
	...
	unsigned int userID					= 0;// declara a variavel que sera usda para armazenar o ID (Codigo) do usuário
	...
	try { //Tentar executar o programa
		...
		}  // THROW joga o erro inesperado para ultima opção (CATCH)
		...
		option = atoi(input.c_str()); // Converte a String (PALAVRA) para um Inteiro (Numérico)

		switch (option) {
			case 1:
			{
				if (EE_EngineConnect() != EDK_OK) { //Da Start a conexão com o EmoEngine
		...
		// Tentando Conectar com as Configurações User (CONFIRMAR)
		std::ofstream ofs(argv[1]);//Pega o Conteúdo das Strings
		...
		while (!_kbhit()) {
			...
			if (state == EDK_OK) {// State for igual ao Define EDK_OK
				...
				if (eventType == EE_EmoStateUpdated) {		//"eventtype=controlador" de adição e remoção de usuarios
					...
					const float timestamp = ES_GetTimeFromStart(eState); // Retorna o tempo em que o capacete está conectado ou zera se desconectado

					//printf("%10.3fs : New EmoState from user %d ...\n", timestamp, userID);
					printf( ES_ExpressivIsBlink(eState),"%s\n");// ES_ExpressivIsBlink(eState) 
					logEmoState(ofs, userID, eState, writeHeader);    //Função de relatório do Emostate
					writeHeader = false;
				}
			}
			else if (state != EDK_NO_EVENT) { //EDK_NO_EVENT = Retorna quando não há um novo evento no EmoEngine
	...
	EE_EngineDisconnect(); // Finaliza conexão com o EmoEngine
	EE_EmoStateFree(eState);  // Libera Memória do Computador referenciado pelo EmoState
	EE_EmoEngineEventFree(eEvent);  // Libera Memória do Computador referenciado pelo EventState
