...
void loop()
{
  val_lido_e = analogRead(entrada_e); //lê o sensor da esquerda
  val_lido_d = analogRead(entrada_d); //lê o sensor da direita
  f=(val_lido_e + val_lido_d)/2; //auxiliar de diferença de luminosidade
//  Serial.print(f);
  if(f <= 100) // luminosidade até 100 (escuro)
  {
    direita_media=f;
    esquerda_media=f;
  }
  if(f <= 300) // luminosidade até 300 (pouco escuro)
  {
    direita_media=f;
    esquerda_media=f;
  }
  if(f <= 500) // luminosidade até 500 (pouco claro)
  {
    direita_media=f;
    esquerda_media=f;
  }
  if(f <= 700) // luminosidade até 700 (claro) 
  {
    direita_media=f;
    esquerda_media=f;
  }
...
//FUNÇOES DE BAIXO NIVEL === ACESSO AO HARDWARE
void ativa_led()
{
  digitalWrite(7, HIGH);
}
