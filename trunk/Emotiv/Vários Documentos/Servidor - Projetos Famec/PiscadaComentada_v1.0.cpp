...
std::cout << "===================================================================" << std::endl;
std::cout << "v1.0 Example to show how to log the EmoState from EmoEngine/EmoComposer." << std::endl;
std::cout << "===================================================================" << std::endl;
...
option = atoi(input.c_str()); //input .c_str()
switch (option) {
case 1:
{
...
int tes;
while (!_kbhit()) {
state = EE_EngineGetNextEvent(eEvent); //API Emotiv - 
// New event needs to be handled
if (state == EDK_OK) {// EDK_OK = Define
...
tes=ES_ExpressivIsBlink(eState);
printf("Blink:%d\n",tes);
do{
if (tes==1){
printf("Piscou \n");
x=x+1;
printf("Contador de Blink e: %d\n",x);
}
else{
printf("No Piscou \n");
}
